...
in.fn <- as.filename("v160_sca_cd4.rds");
...
# drop the durable response level due to insufficient sample size
sca <- sca[, colData(sca)$response != "durable"];
colData(sca)$response <- droplevels(colData(sca)$response);

with(colData(sca), table(response))
